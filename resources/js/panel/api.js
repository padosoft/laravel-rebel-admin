  ...
  async function hydrate(periodKey) {
    ...
    if (ch && nonEmpty(ch.rows) && ch.rows.some((r) => r.sent > 0)) {
      D.channels = ch.rows.map((r) => ({
        channel: r.channel, provider: r.provider || '—', sent: r.sent,
        delivered: r.delivered_rate, fallback: r.fallback_rate, p50: r.latency_p50_ms, p95: r.latency_p95_ms,
        cost: r.cost_amount || 0, cur: r.cost_currency || 'EUR', conv: r.verify_conversion, fraud: !!r.fraud_flag,
      }));
    }
